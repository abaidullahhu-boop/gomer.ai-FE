...
export const researchPosts: BlogPost[] = [
  {
    ...
    title: "How We Built Gaspo Around Prompt Caching",
    excerpt:
      "A deep dive into how Gaspo's SDK, thread engine, and prompt caching architecture work together to cut inference costs by up to 90% without sacrificing agent quality.",
    subtitle:
      "Engineering deep-dive on Gaspo's prompt caching architecture — from SDK design to thread engines and real-world cost savings.",
    date: "June 8, 2024",
    author: "Toni Albert",
    keyTakeaways: [
      "Prompt caching shifts cost from repeated context to a one-time cache write — critical for long-running agent threads.",
      "Gaspo's thread engine structures messages so cache breakpoints land on stable prefixes, not volatile tail content.",
    ...
    intro: [
      "When we started building Gaspo, we knew agents would run long threads — dozens of tool calls, megabytes of context, conversations that span days. Standard LLM pricing makes that economically impossible at scale.",
      "Anthropic's prompt caching gave us a path forward. But caching is not automatic: you have to architect your entire request pipeline around where breakpoints land, what stays stable, and what mutates every turn. This post walks through how we did that in Gaspo.",
    ],
    sections: [
      {
        title: "Why prompt caching matters for agents",
        paragraphs: [
          "A typical Gaspo session might include a 15k-token system prompt, 30k tokens of tool definitions, and a growing conversation history. Without caching, every turn re-processes the full context at input-token rates.",
      ...
      },
      {
        ...
        paragraphs: [
          "Gaspo's thread engine maintains a structured message list with explicit cache control markers. On each turn, the engine assembles the request in a deterministic order: system → tools → stable context → conversation tail.",
...
const request = {
  ...
  messages: conversationHistory, // grows each turn, not cached
      ...
      },
      {
        ...
        paragraphs: [
          "We exposed cache semantics directly in the Gaspo SDK so skill authors do not accidentally bust the cache. Skills declare whether their context is stable (cacheable) or dynamic (per-turn).",
    ...
    faqs: [
      {
        q: "Does prompt caching work with all models Gaspo supports?",
  ...
  },
  {
    ...
    subtitle:
      "Why connecting thousands of integrations to an AI agent breaks naive tool-calling — and how Gaspo routes, filters, and scopes tools in production.",
    date: "March 3, 2024",
    author: "Peter Albert",
    keyTakeaways: [
    ...
    intro: [
      "Gaspo connects to over 3,000 integrations, each with dozens of actions. In theory, that is 100,000+ callable tools. In practice, sending even 1,000 tool definitions to an LLM degrades quality, blows context budgets, and makes the model worse at picking the right action.",
    ...
    sections: [
      ...
      },
      {
        ...
        paragraphs: [
          "Gaspo scopes tools in layers. The workspace defines which integrations are connected and authenticated. Skills declare which integration categories they need. The thread engine activates only the relevant tool subset for the current task.",
    ...
    faqs: [
      {
        q: "How many tools does Gaspo expose per turn?",
