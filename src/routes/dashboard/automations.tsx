        ...
        >
          <div className="mx-auto w-full max-w-[1000px]">
            ...
            <p className="mb-6 max-w-[65ch] text-sm text-muted-foreground">
              What Gaspo is running unattended. Set these up by asking Gaspo in Slack — this page
              reports on them.
            </p>
            ...
            <div className="mb-6 flex gap-1 rounded-md border border-border bg-card p-1">
              {TABS.map((t) => (
                <button
                  ...
                  onClick={() => setTab(t.id)}
                  className={[
                    "gaspo-focus-ring inline-flex min-h-9 flex-1 cursor-pointer items-center justify-center rounded-[5px] px-3 py-1.5 text-sm font-medium transition-colors",
                    tab === t.id
  ...
  return (
    <>
      {rules.length === 0 ? (
        <Empty
          title="No rules yet"
          hint='Ask Gaspo in Slack, e.g. "every night at 2am pause any campaign whose CPA over the last 3 days is above 40".'
        />
      ) : (
  ...
  if (rows.length === 0) {
    return (
      <Empty
        ...
        hint={`Ask Gaspo, e.g. "every Monday at 8am put last week's campaign performance in that sheet".`}
      />
    );
  }
  return (
          ...
          <td className="px-4 py-3">
            {row.spreadsheetUrl ? (
              <a
                ...
                target="_blank"
                rel="noreferrer"
                className="gaspo-focus-ring text-highlight underline underline-offset-2"
              >
  ...
  if (facts.length === 0) {
    return (
      <Empty
        ...
        hint='Tell Gaspo something durable, e.g. "our target ROAS is 3" or "remember our alerts channel is #ads-alerts".'
      />
    );
  }
  return (
    <>
      ...
      <p className="text-sm text-muted-foreground">
        Gaspo uses these in every conversation. To change or drop one, just tell it — e.g. “forget
        our target ROAS”.
      </p>
    </>
  );
  ...
  if (snapshots.length === 0) {
    return (
      <Empty
        ...
        hint={`Ask Gaspo "what's our real ROAS for the last 7 days?" with Stripe connected, and the result is recorded here.`}
      />
    );
  }
  return (
