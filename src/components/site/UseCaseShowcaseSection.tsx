...
function CardStatusBadge({ icon, label }: { icon: ReactNode; label: string }) {
  return (
    <span className="inline-flex h-12 shrink-0 items-center gap-4 rounded-full bg-[#fff] px-6 body-main font-medium text-[#3c3c3c]">
      {icon}
      {label}
    </span>
  );
...
function SlackMention({ children }: { children: ReactNode }) {
  return (
    <span className="inline-block rounded-sm bg-[#ffffff29] px-1 py-0.5 align-baseline whitespace-nowrap body-main text-[#fff]">
      {children}
    </span>
  );
}
