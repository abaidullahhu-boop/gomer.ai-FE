  ...
  organization: {
    ...
    cards: [
      ...
      },
      {
        ...
        gaspoTime: "11:46 AM",
        gaspoMsg: "Done ✅ Updated the pricing page — $79/mo for new. Preview is ready for your review.",
      },
    ],
  },
  code: {
    ...
    <section className="px-6 py-24" style={{ background: "#fff" }}>
      <div className="mx-auto max-w-6xl">
        <h2 className="font-display text-center text-5xl md:text-6xl text-foreground leading-[1.05]">
          ...
          <br />
          Gaspo does: <span className="inline-flex items-baseline gap-3 align-middle">{v.icon}{v.label}</span>
        </h2>
            ...
            <div className="w-full min-w-0">
              ...
              <div className="flex w-full min-w-0 flex-col justify-end gap-2 p-3 sm:p-4">
                ...
                <SlackMessage
                  ...
                  body={data.userMsg}
                />
                {/* gaspo message wrapper - 22 with gradient bg */}
                <GaspoMessage time={data.gaspoTime} body={data.gaspoMsg} />
              </div>
            </div>
          </div>
        </div>
...
function GaspoMessage({ time, body }: { time: string; body: ReactNode }) {
  return (
    <div className="mt-1.5 flex flex-wrap items-stretch gap-1">
      <div className="relative flex self-stretch w-full">
        {/* 22 */}
        <div className="relative flex w-full text-left isolate overflow-hidden slack-message-gaspo gap-2 px-[var(--slack-message-pad-x)] py-[var(--slack-message-pad-y)]">
          {/* 23 - bg mount */}
          <div
            style={{
              isolation: "isolate",
