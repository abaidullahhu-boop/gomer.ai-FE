...
export function SettingsLayout() {
  return (
    <>
      <PageMeta title="Settings — Zundy" description="Manage workspace settings." />
            ...
            <div role="tablist" aria-label="Settings sections" className="flex items-center gap-1">
              {tabs.map((tab) => (
                <NavLink
                  ...
                  className={({ isActive }) =>
                    [
                      "zundy-focus-ring inline-flex min-h-10 cursor-pointer items-center justify-center gap-2 rounded-[7px] border-0 px-4 py-2 text-sm font-medium whitespace-nowrap transition-[background-color,border-color,transform] duration-200 select-none active:scale-[0.98]",
                      isActive
